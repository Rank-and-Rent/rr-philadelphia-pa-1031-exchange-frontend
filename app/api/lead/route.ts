...
function getProjectTypeFromSite(site: string): string {
  const siteToProjectType: Record<string, string> = {
    "rr-philadelphia-pa-1031-exchange": "1031 Exchange",
  };
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://www.1031exchangephiladelphia.com";
    ...
    const payload = {
      ...body,
      projectType: getProjectTypeFromSite("rr-philadelphia-pa-1031-exchange"),
      contractorEmail: process.env.CONTRACTOR_EMAIL || "",
      timestamp: new Date().toISOString(),
      source: process.env.NEXT_PUBLIC_SOURCE || "1031 Exchange Philadelphia Website",
      submitted_at: new Date().toISOString(),
      website_url: siteUrl,
      _meta: {
        site: "rr-philadelphia-pa-1031-exchange",
        route: "/api/lead",
      },
    };
